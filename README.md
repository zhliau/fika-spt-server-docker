...
```
docker pull ghcr.io/zhliau/fika-spt-server-docker:3.10.2
```
...
- Update the image version tag e.g. `fika-spt-server-docker:3.9.8` to `fika-spt-server-docker:3.10.2`
...
| `FIKA_VERSION`            | v2.3.1  | Override the fika version string to grab the server release from. The release URL is formatted as `https://github.com/project-fika/Fika-Server/releases/download/$FIKA_VERSION/fika-server.zip` |
...
```bash
# Server binary built using SPT Server 3.10.2 git tag, image tagged as fika-spt-server:latest
# Downloads and validates Fika version v2.3.1

VERSION=latest FIKA_VERSION=v2.3.1 SPT_SHA=3.10.2 ./build
```
